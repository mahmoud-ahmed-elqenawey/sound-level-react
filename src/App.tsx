      ...
      setDepartments((prev) =>
        prev.map((dept) => ({
          ...
          currentNoise: Math.max(
            20,
            dept.currentNoise + (Math.random() - 0.5) * 4
          ),
          status:
          ...
          <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg sm:text-2xl font-bold text-yellow-600 mt-0.5 sm:mt-1">
                  {warningDepartments}
                </p>
                <p className="text-xs text-gray-500 mt-0.5 sm:mt-1 hidden sm:block">
                  تنبيهات تحذيرية
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                {departments.map((dept) => (
                  <div
                    key={dept.id}
                    className={`p-3 sm:p-4 rounded-lg border-2 transition-all duration-300 ${getStatusColor(
                      dept.status
                    )}`}
                  >
            ...
            {departments.slice(0, 3).map((dept) => (
              <div key={dept.id} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <MiniChart
                    data={
                      viewMode === "hourly" ? dept.hourlyData : dept.dailyData
                    }
                    color={getProgressColor(dept.status)}
                  />
                </div>
